...
//    finalDS.show()
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      ...
      ):::columnNames.tail.map(colname => StructField(colname,DoubleType,false))
    )
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val prefix_primary = List("t01", "t03", "t11", "t1801", "t1803")
    val prefix_work = List("t05", "t1805")
    val prefix_other = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    def isCategory(columnName: String, prefix: List[String]):Boolean =
    {
      for (pre <- prefix) {
    ...
    for (colname <- columnNames) {
      if (isCategory(colname,prefix_primary)) {
        primary += col(colname)
      } else if (isCategory(colname,prefix_work)) {
        work += col(colname)
      } else if (isCategory(colname,prefix_other)) {
        other += col(colname)
      }
    }
  ...
  def timeUsageSummary(
    ...
    val workProjection: Column = (workColumns.reduce((x1, x2) => x1 + x2).cast(DoubleType) / 60).as("work")

    val otherProjection: Column =  (otherColumns.reduce((x1, x2) => x1 + x2).cast(DoubleType) / 60).as("other")

    df
      .select(workingStatusProjection, sexProjection, ageProjection, primaryNeedsProjection, workProjection, otherProjection)
      .where($"telfs" <= 4) // exclude people that are not employable
  }
